# Legalwork cache optimization technical documentation

This article records the cache optimization design, implementation location, and implementation of the current Legalwork runtime of DeepSeek GUI.
...
Legalwork's cache optimization serves four goals:
...
- Let the GUI only bear the responsibility of HTTP/SSE calls, and consolidate the caching discipline inside Legalwork.
...
Legalwork therefore treats token optimization as a combined strategy, not a
...
Legalwork borrowed Reasonix’s cache-first design, but adapted it to the GUI scenario:

- The GUI does not spell prompt, and does not make cache judgments in renderer or main process.
- `legalwork serve` is the only request exit, and cache-related strategies are placed inside the runtime.
...
Legalwork uses a separate stable system prompt word file:

- `legalwork/src/prompt/legalwork-system-prompt.ts`
...
Legalwork manages system prompts, tools, and pinned constraints through `ImmutablePrefix`
...
The toolset itself is part of the prompt prefix. Legalwork will do the following before sending a request:
...
- `legalwork/src/adapters/model/deepseek-compat-model-client.ts`
- `legalwork/src/cache/tool-catalog-fingerprint.ts`
- `legalwork/src/loop/agent-loop.ts`
...
Legalwork currently performs a shared model history repair on messages at the model request boundary:
...
- `legalwork/src/domain/model-history-repair.ts`
- `legalwork/src/adapters/model/deepseek-compat-model-client.ts`
- `legalwork/src/loop/agent-loop.ts`

Legalwork will also do a layer of Reasonix-style history hygiene at model request boundaries:
...
- `legalwork/src/loop/request-history-hygiene.ts`
- `legalwork/src/loop/agent-loop.ts`

Repeat-loop guard is also enabled within the same turn:

- The third identical `(toolName, arguments)` will be suppressed.
- Legalwork will write an error `tool_result` to let the model converge to a narrower query or explain the reason.
...
- `legalwork/src/loop/tool-storm-breaker.ts`
- `legalwork/src/loop/agent-loop.ts`
...
- `legalwork/src/domain/model-history-repair.ts`- `legalwork/src/services/thread-service.ts`
...
Legalwork's cache hit statistics preferentially use DeepSeek's native usage field:
...
- `legalwork/src/telemetry/usage-counter.ts`
- `legalwork/src/domain/usage.ts`

Legalwork will also use a single round of real `prompt_tokens` as the compaction pressure for the next request.
...
- `legalwork/src/loop/agent-loop.ts`
- `legalwork/src/loop/context-compactor.ts`
...
- `legalwork/src/services/usage-service.ts`
- `legalwork/src/server/runtime-factory.ts`
...
Legalwork has covered these key behaviors:
...
- `legalwork/tests/cache.test.ts`
- `legalwork/tests/model-client.test.ts`

### 2. Runtime usage API

The GUI uses a unified interface to read usage through Legalwork:
...
Legalwork actual measurement results on 2026-06-02:
...
This document emphasizes a production-level constraint: cache optimization belongs to Legalwork, not the GUI.
...
- Architecture overview: `docs/legalwork-architecture.md`
- Legalwork Contributing Guide: `docs/legalwork-contributing.md`
- Legalwork usage instructions: `legalwork/README.md`
