# Legalwork GUI 单运行时方案

本文记录 DeepSeek GUI 现在应该如何围绕一个专门服务 GUI 的
Legalwork 改造。结论先说清楚：GUI 只保留一个 agent，唯一 ID 是
`legalwork`；Code、Write、连接手机都通过同一条 `legalwork serve`
...
- `legalwork/src/prompt/legalwork-system-prompt.ts` 是稳定前缀。它只放长期
  不变的 Legalwork 运行契约，不能放 workspace、时间戳、文件片段、选中文本、
  用户动态信息或一次性工具结果。
...
超过 90%。2026-06-02 的真实 Legalwork 临时线程验证：
...
- 工具集合 mutation gate：新增工具允许 append，编辑、重排、删除工具时要求
  restart 或新会话边界，避免热前缀突然全量 miss。当前 Legalwork 已排序工具
...
Renderer 只应展示 Legalwork。需要删除或保持删除的 UI 面包括：
...
- 设置页 provider selector：Settings -> Agents 直接展示 Legalwork 配置，
...
- 删除 Legalwork 之外的 diagnostics/importer 模块。用户要的是可用的单
  agent，不是运行时检测中心。

主进程现在只需要：

- `legalworkRuntimeAdapter`：启动/停止 `legalwork serve`、同步 config、
  计算 base URL、附加 auth header。
- `runtimeRequestViaHost`：确保 Legalwork running 后转发 `/v1/*`。
- `startSse/stopSse`：按 `threadId + sinceSeq` 转发 Legalwork SSE。
...
- 旧 `deepseek`/`agents.codewhale` 的 port、autoStart、API key、
  base URL、runtime token、approval、sandbox 会种到 `agents.legalwork`。
...
- 连接手机（内部旧名 Claw）旧 `agentThreadIds.codewhale/reasonix` 只折叠成
  `agentThreadIds.legalwork`，不保留 per-agent map。

## Code / Write / 连接手机如何走 Legalwork

- Code：`LegalworkRuntimeProvider` 负责 list/create thread、send turn、
  steer、interrupt、compact、approval、SSE 映射。Chat UI 不知道旧
  provider。
- Write：写作助手和 inline completion 读取同一份 Legalwork API key /
  base URL 配置。Write thread registry 只把写作线程识别为 Legalwork
  thread，不再区分 Reasonix 会话。
- 连接手机：定时任务、飞书/Lark/微信、IM webhook 创建或复用 Legalwork thread。
  代码内部仍沿用 `claw` route / settings key / runtime 文件名，作为旧命名兼容。
  `threadId` / `localThreadId` 字段只作为旧 settings 兼容字段存在，真正
  当前映射写入 `agentThreadIds.legalwork`。

## CodeWhale 功能等价面

替换 CodeWhale 不是只保留聊天。Legalwork 的 GUI HTTP 面必须覆盖旧
...
- `POST /v1/sessions/{id}/resume-thread` 沿用旧 CodeWhale resume 路径。
  Legalwork 优先从同名 thread 恢复；没有 thread 时从 session snapshot
...
- `GET /v1/usage?group_by=thread|day` 返回累计 token、turn、cache hit 数据。
  Workbench 首页和 composer 底部只消费 Legalwork usage，不再打开 runtime
  insights 面板。
...
Legalwork 包按 ports & adapters 组织：
...
新增能力时优先加 Legalwork tool 或 HTTP endpoint，不新增 GUI 内第二个
agent。
...
4. 连接手机能保存设置、运行手动 task、把 thread id 写回 Legalwork mapping。
5. Settings -> Agents 只看得到 Legalwork，没有 provider switch、runtime
