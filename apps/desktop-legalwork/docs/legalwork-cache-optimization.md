...
本文记录 DeepSeek GUI 当前 Legalwork 运行时的缓存优化设计、实现位置、
...
Legalwork 借鉴了 Reasonix 的 cache-first 设计，但按 GUI 场景做了收束：

- GUI 不拼 prompt，不在 renderer 或 main process 做缓存判断。
- `legalwork serve` 是唯一请求出口，缓存相关策略都放在运行时内部。
...
Legalwork 通过 `ImmutablePrefix` 管理系统 prompt、tools、pinned constraints
...
工具集合本身就是 prompt prefix 的一部分。Legalwork 在发送请求前会统一做：
...
Legalwork 当前在模型请求边界对消息做一层共享的 model history repair：
...
Legalwork 也会在模型请求边界做一层 Reasonix 风格的 history hygiene：
...
- Legalwork 会写入一个 error `tool_result`，让模型收敛到更窄的查询或解释原因。
...
- `legalwork/src/domain/model-history-repair.ts`
- `legalwork/src/services/thread-service.ts`
...
Legalwork 的缓存命中统计优先使用 DeepSeek 原生 usage 字段：
...
Legalwork 也会把单轮真实 `prompt_tokens` 作为下一次请求的 compaction pressure。
...
GUI 通过 Legalwork 使用统一接口读取 usage：
...
这个文档强调一个产品级约束：缓存优化属于 Legalwork，而不是 GUI。
...
- 架构总览：`docs/legalwork-architecture.md`
- Legalwork 贡献指南：`docs/legalwork-contributing.md`
- Legalwork 使用说明：`legalwork/README.md`
