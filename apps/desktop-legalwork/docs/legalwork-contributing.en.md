# Legalwork Contribution Guide: Architecture, design patterns and code organization ideas

> Applicable to: Any engineer who wants to contribute code to the `legalwork/` package.
> Reading path: For your first contribution, you can read the entire text in order; come back later to find specific chapters.
> Supporting documents:[`legalwork-architecture.md`](./legalwork-architecture.md)
...
**Legalwork is a strict implementation of Ports & Adapters + Functional Core / Imperative
...
Legalwork's directory itself is a hexagonal physical layout:
...
Legalwork uses multiple classic patterns simultaneously, which reinforce each other rather than conflict.
...
Definition (`legalwork/src/ports/model-client.ts`):
...
Implementation (`legalwork/src/adapters/model/deepseek-compat-model-client.ts`)
Parse HTTP+SSE into a sequence of `ModelStreamChunk`.

Tests (`legalwork/tests/ports.test.ts`) are injected directly with `makeFakeModel`
...
```ts
// legalwork/src/cache/immutable-prefix.ts
...
```ts
// legalwork/src/services/turn-service.ts
...
```ts
// legalwork/src/contracts/items.ts
...
`legalwork/src/contracts/events.ts`). This discipline allows SSE
...
```ts
// legalwork/src/loop/agent-loop.ts
...
Test case (`legalwork/tests/loop.test.ts`):
...
```ts
// legalwork/src/services/runtime-event-recorder.ts (excerpt)
...
of `legalwork/src/adapters/file/file-thread-store.ts`
...
- `legalwork/src/server/routes/index.ts`:`buildRouter(runtime)`
  Routing table + authentication + handlers are installed together;
- serve startup files such as `legalwork/src/server/serve.ts`: put all
  The port implementation (in-memory or file-backed) is installed and plugged into `buildRouter`.
- `legalwork/src/cli/serve.ts` is the composition root of CLI and is only responsible for
...
| --- | --- | --- |
| contracts (zod) | Schema parsing and validation failed | `legalwork/tests/contracts.test.ts` |
| domain/cache | Pure functional behavior | `legalwork/tests/cache.test.ts` |
| ports (in-memory) | interface contract | `legalwork/tests/ports.test.ts` |
| loop | Run full loop with in-memory fakes | `legalwork/tests/loop.test.ts` |
| server | Use ephemeral port + fetch / EventSource to run HTTP contract | `legalwork/tests/http-server.test.ts` |
| adapters (file) | tmp directory, append / atomic / corrupted JSONL fault tolerance | `legalwork/tests/loop.test.ts` (FileSessionStore part) |
...
- `docs/legalwork-architecture.md` describes "why it is designed this way";
- `legalwork/README.md` description CLI/env/data dir;
- `docs/legalwork-contributing.md` (this article) describes "how to contribute";
- `docs/AGENTS.md` describes which files should be changed when extending.

**Contribution Rules**:

- Any new/modified contract must update the `legalwork/README.md` side at the same time
  point table;
...
1. **contracts**:`legalwork/src/contracts/items.ts` is already supported
   `tool_call` / `tool_result`, **No need to change**.
2. **domain**:`legalwork/src/domain/item.ts` already exists
   `makeToolCallItem` / `makeToolResultItem`, **no need to change**.
3. **adapters**: in `legalwork/src/adapters/tool/local-tool-host.ts`
   Add a `LocalTool`:
...
4. **adapters barrel**: in `legalwork/src/adapters/tool/local-tool-host.ts`
   Barrel already exists; just append `shellTool` to the `defaultLocalTools` array.
5. **Test**: Add one to `legalwork/tests/ports.test.ts`
   `runs a shell tool with approval` test.
6. **Documentation**: Described in the default tools list of `legalwork/README.md`.
...
2. **adapters**:`legalwork/src/adapters/model/openai-compat-model-client.ts`
   Create a new one (refer to the SSE parsing logic of `deepseek-compat-model-client.ts`).
3. **adapters barrel**:`legalwork/src/adapters/index.ts` exports new classes.
4. **settings**: in `legalwork/src/contracts/...` or the caller
   `ServeOptionsSchema` adds `modelProvider` field (if you need to switch).
5. **composition root**: serve in `legalwork/src/server/...`
   Which adapter to inject is selected according to the settings in bootstrap.
6. **Test**: `legalwork/tests/ports.test.ts` Add model behavior test;
   `legalwork/tests/http-server.test.ts` adds end-to-end testing.
7. **Document**: `legalwork/README.md` The endpoint table remains unchanged, but
...
1. **contracts**:`legalwork/src/contracts/events.ts`:
...
3. **loop**: in `legalwork/src/loop/context-compactor.ts` or
   The appropriate location for `agent-loop.ts` is `await this.opts.events.record({ kind: 'compaction_progress', ... })`.
4. **server**: SSE of `legalwork/src/server/routes/events.ts`
   The encoder is already based on `RuntimeEvent.kind` routing and does not need to be changed.
5. **renderer mapping** (GUI side): `src/renderer/src/agent/legalwork-runtime.ts`
   Dispatch to `sink.onCompactionProgress(...)` in `subscribeThreadEvents`
   (if needed). **This is a GUI package, not legalwork**.
6. **Test**:
   - `legalwork/tests/contracts.test.ts`: Add variant parsing test;
   - `legalwork/tests/loop.test.ts`: Add emit assertion.
7. **Document**:`legalwork/README.md` The endpoint table remains unchanged, only SSE events
...
1. **ports**:`ThreadStore` / `SessionStore` is already an interface,
   **No changes required**.
2. **adapters**:`legalwork/src/adapters/sqlite/sqlite-thread-store.ts`
   Create new and implement `ThreadStore`.
3. **adapters barrel** + **composition root** in serve bootstrap
   Switch store type.
4. **Test**: `legalwork/tests/ports.test.ts` Add a copy for SQLite
...
- [ ] run `cd legalwork && npm run typecheck && npm test`
- [ ] run `cd .. && npm run typecheck && npm test`
- [ ] Do not modify any files other than those described in `docs/legalwork-architecture.md`
      to "bypass" typecheck
...
- [ ] When the public contract is changed, update the `legalwork/README.md` endpoint table
- [ ] When references/acknowledgments change, update root directories `README.md` and `README.en.md`
- [ ] When adding a new SSE event, add a line to `docs/legalwork-architecture.md`- [ ] in the borrow map of `docs/legalwork-architecture.md` (as in the future
...
- **Not familiar with zod discriminated union**: refer to `legalwork/src/contracts/items.ts`
  And `legalwork/src/contracts/events.ts`, see how to write the existing variant.
...
> **Legalwork is a strict implementation of Ports & Adapters + Functional Core /
