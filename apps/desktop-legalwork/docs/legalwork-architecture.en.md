# Legalwork GUI single-runtime architecture

This document describes how DeepSeek GUI should now be organized around one dedicated runtime,
`Legalwork`, that serves the GUI through a single HTTP/SSE boundary.
The conclusion is clear up front: the GUI keeps one agent with the only ID
`legalwork`; Code, Write, and Connect phone all flow through the same `legalwork serve`
HTTP/SSE boundary.
...
Main process
  ...
  process/config/port/token management only
        |
        v
legalwork serve (TypeScript package)
  /health
  /v1/threads
  /v1/threads/{id}/turns
...
Inside `legalwork`, the cache-first agent loop is adopted from Reasonix (`immutable` prompt
prefix, append-only log, bounded LRU/TTL cache, inflight cleanup, steering queue,
context compaction, usage/cache telemetry).

## Cache-hit optimization

Legalwork cache-hit metrics should be computed and optimized using DeepSeek native fields first:
...
- `legalwork/src/prompt/legalwork-system-prompt.ts` is the stable prefix.
  It may only contain long-lived Legalwork run contract content and must not include
...
- Tool-collection mutation policy: adding tools should be append-only; edit/reorder/remove
  ...
  Current Legalwork canonicalizes schema, but this mutation policy still needs explicit product-level
  enforcement.
...
Renderer should only expose Legalwork. The UI sections listed below should be removed or
kept removed:

- Agent switcher: `AgentSwitcher` is no longer shown; `AGENT_CATALOG` only includes `legalwork`.
...
- Settings provider selector: `Settings -> Agents` directly edits Legalwork config including:
...
- Remove diagnostic/importer modules unrelated to Legalwork.

Main process now only needs:

- `legalworkRuntimeAdapter`: start/stop `legalwork serve`, sync config, calculate base URL,
and append auth headers.
- `runtimeRequestViaHost`: forward `/v1/*` after ensuring Legalwork is running.
- `startSse` / `stopSse`: forward Legalwork SSE streams keyed by `threadId + sinceSeq`.
...
- `agentProvider: codewhale | reasonix | deepseek-runtime` normalizes to `legalwork`.
...
- Old `agents.reasonix` values for `apiKey`, `baseUrl`, `model`, and `autoStart`
  are also migrated to `agents.legalwork`.
...
- Legacy Connect phone fields (internally still named Claw) `agentThreadIds.codewhale` and `agentThreadIds.reasonix` are collapsed
  to `agentThreadIds.legalwork`; per-provider maps are not retained.

## Code / Write / Connect phone flows under Legalwork

- Code: `LegalworkRuntimeProvider` handles list/create thread, send turn,
  steer, interrupt, compact, approval, and SSE mapping.
  Chat UI does not directly know about old providers.
- Write: writing assistant and inline completion share the same Legalwork API key/base URL.
  Write thread registry identifies write threads as Legalwork threads only, with no Reasonix distinction.
- Connect phone: scheduled tasks, Feishu/Lark/WeChat, and IM webhooks create or reuse Legalwork threads.
  ...
  canonical mapping is written to `agentThreadIds.legalwork`.

## Functional parity from CodeWhale in GUI HTTP path

Replacing CodeWhale is not only preserving chat.
Legalwork GUI HTTP must expose the same capabilities previously exposed through CodeWhale:
...
- `POST /v1/sessions/{id}/resume-thread` follows the previous CodeWhale resume path.
  Legalwork should first attempt same-name thread restore, then session snapshot/JSONL reconstruction,
...
- `GET /v1/usage?group_by=thread|day` returns accumulated token/turn/cache-hit counters.
  Workbench home and composer footer consume Legalwork usage only and do not open runtime
  insight panels.
...
Legalwork packages are organized by ports & adapters:
...
and forwards results. When adding capability, add Legalwork tool or HTTP endpoint first,
...
4. Connect phone can save settings, run manual tasks, and write thread IDs back to Legalwork mapping.
5. `Settings -> Agents` shows only Legalwork, with no provider switch and no runtime diagnostics/
