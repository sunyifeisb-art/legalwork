# Legalwork Agent 与模型配置说明

本文说明 DeepSeek GUI / Legalwork 的本地配置文件在哪里、哪些字段由 UI 管理、哪些字段适合手工扩展，以及模型上下文压缩阈值应该如何配置。
...
1. GUI settings
   ...
   Agent 运行时设置在 `agents.legalwork` 下，例如端口、data dir、默认模型、审批策略、sandbox、token economy 等。多数用户通过设置页修改这些字段。

2. Legalwork runtime config

   这是 Legalwork 本地运行时读取的高级配置文件。默认路径是：

   ```text
   ~/.legalwork/legalwork/config.json
   ```

   如果 `agents.legalwork.dataDir` 改成了别的目录，实际路径就是：

   ```text
   <dataDir>/config.json
   ```

   `legalwork serve --config <path>` 可以显式指定配置文件；如果没有指定，Legalwork 会尝试读取 `{dataDir}/config.json`。

## 启动时的读取顺序

GUI 启动 Legalwork 时会按下面的顺序合并配置。

1. GUI 读取 `deepseek-gui-settings.json`，得到 `agents.legalwork` 和通用 provider 配置。
2. GUI 在启动 Legalwork 前同步 `<dataDir>/config.json`，写入 UI 管理的 token economy、默认压缩摘要参数、默认模型 profiles、runtime tuning、MCP search 和附件能力。
3. Legalwork serve 读取 `<dataDir>/config.json` 或 `--config` 指定的文件。
4. CLI 参数和环境变量会覆盖 `serve` 里的基础启动字段，例如 `--model`、`--port`、`LEGALWORK_MODEL`、`LEGALWORK_PORT`。
...
Legalwork 内置 DeepSeek V4 默认模型画像：
...
普通 Agent 运行时配置由 GUI settings 的 `agents.legalwork` 管理。主要字段：
...
设置页会保存这些字段。GUI 模式下默认模型以 `agents.legalwork.model` 为准；`config.json` 里的 `serve.model` 更适合 standalone `legalwork serve` 使用，因为 GUI 启动时会把设置页里的模型作为启动参数传给 Legalwork。
...
3. 如果要把自定义模型作为 GUI 默认模型，把 `agents.legalwork.model` 改成该模型 ID。
4. 重启 Legalwork runtime，让新配置生效。
...
- GUI 同步 `<dataDir>/config.json`：`src/main/legalwork-process.ts`
- Legalwork config schema：`legalwork/src/config/legalwork-config.ts`
- 模型 profile 解析：`legalwork/src/loop/model-context-profile.ts`
- 上下文压缩器：`legalwork/src/loop/context-compactor.ts`
- serve 解析入口：`legalwork/src/cli/serve.ts`
- 示例配置：`legalwork/config.example.json`
