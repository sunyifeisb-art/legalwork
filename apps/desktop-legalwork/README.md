<p align="center">
  <img src="src/asset/img/legalwork.png" width="96" alt="LegalWork 图标">
</p>
...
> 一个面向法律专业人士和文档密集型工作流的本地桌面工作台。基于 Legalwork Agent 运行时，专为**文档审查**、**合规分析**、**案例研究**和**法律文书起草**而设计——让每一个 token 都花在需求、事实、法规和结论上。

[![GitHub release](https://img.shields.io/github/v/release/XingYu-Zhong/legalwork?label=github)](https://github.com/XingYu-Zhong/legalwork/releases)
[![License](https://img.shields.io/github/license/XingYu-Zhong/legalwork)](./LICENSE)

LegalWork 是一个面向法律专业人士、合规人员和文档密集型工作流的本地桌面工作台。它以 Legalwork 为唯一运行时，把终端里的智能体体验整理成更好用的桌面应用：打开案件工作区，启动审查，实时观看 Agent 阅读文档、调用工具、产出结构化结论。

这个项目的目标是让 AI 驱动的法律助手能够处理真实的案件档案、法规条文、合同和合规文档——而不是再造一个通用聊天壳。Legalwork 的核心优势是高 Token ROI：同样的上下文预算，少浪费在重复前缀、庞大工具目录和失控输出上，多投入到真正推动任务完成的信息里。

---

<p align="center">
  <a href="src/asset/img/code.mp4">
    <img src="src/asset/img/code.gif" width="410" alt="LegalWork Code 模式演示">
  </a>
  <a href="src/asset/img/write.mp4">
    <img src="src/asset/img/write.gif" width="410" alt="LegalWork 写作模式演示">
  </a>
</p>
...
## Legalwork 为什么 Token ROI 高

Legalwork 把“省 token”做成 agent loop 的默认行为，而不是事后补救。它不只是压缩文本，更是在每一轮调用前判断哪些信息值得进入上下文。

| Legalwork 优势 | Token ROI 来源 |
| --- | --- |
...
结果是：Legalwork 更适合真实项目里的长任务、长会话和多工具协作。它把模型注意力留给高价值上下文，让用户用同样的 API 预算换到更多有效推进。

## 我们做了什么

- 把 Legalwork 本地运行时封装进桌面应用，默认可以自动启动和管理。
...
- 做了学习迭代、计划面板、线程 Todo、目标追踪和代码审查，让长期交互能够沉淀为可审计、可回滚的记忆与工作方法。
- 做了 **Agent 长期记忆**系统，让 Agent 记住用户偏好、工作流程和项目上下文，基于文件持久化存储，支持安全策略和置信度评分。
- 做了**排队/引导发送**交互，任务执行中发送新消息可选择排队等待或立即中断引导。
- 提供了**Flint Chart**可选图表 MCP 支持，按需安装后 Agent 可自动生成 Vega-Lite / ECharts / Chart.js 图表。
...
- **Agent 长期记忆**：Agent 可自动记忆用户偏好、工作流程和项目上下文。支持 user/workspace/project 三级隔离、7 种分类与置信度评分、always/relevant 双召回策略，并提供密钥/敏感信息安全策略和独立记忆管理面板。
- **学习迭代**：在软件空闲时增量学习对话、用户上传文件、Agent 产出、过程经验、记忆、知识库与 Skill 使用结果；有长期价值的内容会以主题化命名的 Markdown 沉淀到本地知识库，并保留完整报告与回滚能力。
- **计划与 Todo**：`/plan` 会生成可编辑的计划文件，右侧计划面板会同步线程 Todo，方便把长任务拆成可跟踪步骤。
...
- **排队/引导发送**：任务执行中发送消息时，可选择「排队」加入队列等待，或「引导」立即中断当前任务直接输入，灵活控制交互节奏。
- **变更审查**：内联 diff 和侧边审查面板会记录智能体产生的文件改动，便于在应用内完成 review。
- **权限可控**：支持只读、工作区可写、完全访问等模式，并可配置工具调用前是否需要审批。
- **运行时托管**：默认使用内置 Legalwork；也可以在设置中指定自己的 `legalwork` 可执行文件。
- **Skill 与 MCP**：在图形界面中创建 Skill、保存 MCP 配置、添加常用工具，并打开对应目录继续管理。支持按需安装 Flint Chart 可选 MCP 生成图表。
- **可开关的 agent 扩展能力**：Legalwork 通过配置开关逐步启用 MCP、Web fetch/search、Skills、独立 CLI、图片附件、跨会话 Memory 和子 agent 委派；设置页会显示运行时实际上报的能力与诊断状态。
- **连接手机**：可开启独立于普通聊天的后台 Agent，当前支持飞书 / Lark / 微信接入、IM webhook / relay，以及按计划自动执行任务。
- **定时任务**：创建一次性、每日、间隔或手动任务，指定工作区、模型和推理强度，让 Legalwork 在电脑唤醒时自动执行。
- **Write 写作模式**：独立管理 `~/.legalwork/write_workspace` 和自定义写作空间，读取 Markdown 文件树，支持 live Markdown 编辑、相对图片预览、DeepSeek FIM 短补全 / 灵感长补全（可用跨文本 BM25 + 关键词检索增强）、当前文档导出为 `HTML / PDF / DOC / DOCX`，以及选中文本后直接唤起 inline 写作助手。
- **高 Token ROI**：Legalwork 会稳定 prompt 前缀、跟踪 DeepSeek 原生缓存命中、按需压缩上下文和工具输出，并用 MCP search 渐进发现工具，把 token 留给需求、代码、决策和结果。
...
**Legalwork**（位于 `legalwork/` 目录）。它不是一个临时聊天壳，而是把模型能力沉到
更深的本地运行时里，让它能承载更长的上下文、更复杂的工具调用和更持续的
法律与项目协作。技术上，Legalwork 是一个独立的 TypeScript 包，启动本地 HTTP/SSE
服务作为 GUI 与 agent loop 之间的唯一边界。

Legalwork 的核心理念是提高每一个 token 的 ROI。对用户来说，同样的上下文预算
应该尽量花在需求、代码、决策和结果上，而不是重复的工具 schema、失控的
工具输出、无效历史或已经可以被缓存复用的前缀上。它适合的不是一次性问答，
而是反复读写项目、持续调用工具、需要长期上下文的真实工作流。

Legalwork 集成了已被验证的设计：
...
> 致谢：感谢 Reasonix 团队提供的可运行参考。Legalwork
> 的几乎全部性能特征——cache hit 率、token replay、断线重连、
> 审批中断——都可以追溯到该项目。具体设计取舍与借鉴映射
> 详见 [`docs/legalwork-architecture.md`](docs/legalwork-architecture.md)。

如果你想专门了解 Legalwork 如何做缓存优化，包括稳定前缀、工具 schema
...
Legalwork 的大块 agent 能力采用 feature flag 管理：`capabilities.mcp`
...
委派。`legalwork run` / `legalwork chat` / `legalwork exec` 可脱离 GUI 运行；GUI 的设置页
...
provider，settings 会在读取时迁移到 `agents.legalwork`，再次保存后
只保留 Legalwork 配置。

完整的端点、CLI flag、环境变量、data dir 布局、SSE 事件 schema
见 [`legalwork/README.md`](legalwork/README.md)。
...
LegalWork 现在以 **Code** 和 **写作** 两个主工作台为核心，并提供
**连接手机**、**定时任务**、**插件 / Skill / MCP** 等入口。它们共享同一套
Legalwork 运行时与设置，但会话、工作区和界面布局彼此独立，可按任务随时切换。
...
<p align="center">
  <img src="src/asset/img/codemode.png" alt="LegalWork Code 模式" width="860">
</p>

- 按工作区管理多个 Agent 会话，实时查看推理、工具调用与文件变更。
- 支持内联 diff、变更审查面板，以及只读 / 工作区可写 / 完全访问等权限策略。
- 支持学习迭代、`/plan` 计划、右侧计划面板、线程 Todo 和 `/goal` 长期目标，让复杂任务可以持续沉淀、计划和执行。
...
<p align="center">
  <img src="src/asset/img/writemode.png" alt="LegalWork Write 模式" width="860">
</p>

- 管理 `~/.legalwork/write_workspace` 与多个自定义写作空间，左侧文件树支持新建、重命名与删除。
...
把 Legalwork 连接到手机和 IM 的后台自动化入口，让 Agent 在普通桌面聊天之外持续处理消息与定时任务。

<p align="center">
  <img src="src/asset/img/clawmode.png" alt="LegalWork 连接手机" width="860">
</p>
...
- 定时任务可设置一次性、每日、间隔或手动运行，任务会创建独立 Legalwork thread，并按配置发送 prompt。
...
前往 [GitHub Releases](https://github.com/XingYu-Zhong/legalwork/releases) 下载最新版本：
...
```bash
git clone https://github.com/XingYu-Zhong/legalwork.git
cd legalwork/apps/desktop-legalwork
npm install
npm run dev
```
...
Write 模式的目标是把 LegalWork 从“代码/聊天工作台”扩展成真正可长期写作的桌面工作区。实现时参考了本地 `openhanako` 项目中的几个方案：
...
- 打开“设置 -> 应用 -> 已安装的应用”，找到 `LegalWork` 并卸载。
- 或在“控制面板 -> 程序和功能”中卸载。
- 也可以运行安装目录中的卸载程序。

Windows 安装器默认会创建开始菜单和桌面快捷方式。安装包不会强制固定到任务栏；如需固定，可在开始菜单中右键 `LegalWork` 并选择固定。

### macOS

- 将 `LegalWork.app` 从“应用程序”移到废纸篓。
...
Legalwork 数据默认位于 `~/.legalwork/legalwork` 或应用数据目录下的 Legalwork data dir。删除前请确认其中没有你还需要的会话、MCP 或 Skill 配置。

---

## 更新

- 普通用户：可在设置页检查 GUI 更新，更新来源为 [GitHub Releases](https://github.com/XingYu-Zhong/legalwork/releases) 上发布的新版本；也可以直接前往该页面下载安装包。
...
| 文档 | 内容 |
| --- | --- |
| [docs/legalwork-architecture.md](docs/legalwork-architecture.md) | Legalwork 单运行时方案、GUI 拆改范围、HTTP/SSE 合约、旧 agent 拆除说明 |
| [docs/legalwork-cache-optimization.md](docs/legalwork-cache-optimization.md) | Legalwork 缓存优化、token economy、MCP search、工具输出压缩与用量收益统计 |
| [docs/legalwork-contributing.md](docs/legalwork-contributing.md) | Legalwork 贡献指南：六边形架构、设计模式（Ports & Adapters / Functional Core Imperative Shell / 事件溯源 / 显式 DI / Composition Root）、4 个典型 PR 场景 |
| [legalwork/README.md](legalwork/README.md) | Legalwork 包：CLI、env、data dir、HTTP API |
...
- 所有为 LegalWork 提交 issue、建议、代码和文档的贡献者。
