...
release_export_update_channel() {
  ...
  export LEGALWORK_UPDATE_CHANNEL="${RELEASE_CHANNEL}"
...
release_load_local_env() {
  local env_file="${LEGALWORK_RELEASE_ENV:-${DEEPSEEK_GUI_RELEASE_ENV:-}}"
...
release_compute_version() {
  ...
  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    ...
    RELEASE_NAME="LegalWork ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_export_app_version() {
  ...
  export LEGALWORK_APP_VERSION="${RELEASE_VERSION}"
  export DEEPSEEK_GUI_APP_VERSION="${RELEASE_VERSION}"
  cyan "  App:     ${LEGALWORK_APP_VERSION}"
}
...
release_clean_dist_artifacts() {
  ...
  rm -f "${ROOT}"/dist/legalwork-* "${ROOT}"/dist/LegalWork-* "${ROOT}"/dist/DeepSeek-GUI-* "${ROOT}"/dist/DeepSeek\ GUI-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
}
