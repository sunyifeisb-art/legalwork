...
import {
  legalworkConfigPathForDataDir,
  readLegalworkConfigFile,
  readOptionalLegalworkConfigFile,
  type LoadedLegalworkConfig
} from '../config/legalwork-config.js'
import { normalizeModelEndpointFormat } from '../contracts/model-endpoint-format.js'

/**
 * Parse the `legalwork serve` command line into validated options.
 *
  ...
  const merged: ServeOptions = {
    ...
    host:
      typeof raw.host === 'string'
        ? raw.host
        : env.LEGALWORK_HOST ?? configServe.host ?? DEFAULT_SERVE_OPTIONS.host,
    port:
    ...
    model:
      typeof raw.model === 'string'
        ? raw.model
        : env.LEGALWORK_MODEL ?? configServe.model ?? DEFAULT_SERVE_OPTIONS.model,
    approvalPolicy:
 ...
 * main process when Legalwork is started programmatically.
 */
...
export const SERVE_USAGE = `legalwork serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedLegalworkConfig | null {
  const explicitConfigPath =
    stringFlag(raw, 'config') ??
    stringFlag(raw, 'config-file') ??
    env.LEGALWORK_CONFIG
  if (explicitConfigPath) {
    return readLegalworkConfigFile(explicitConfigPath)
  }
  const dataDir = dataDirFromRawOrEnv(raw, env)
  return readOptionalLegalworkConfigFile(legalworkConfigPathForDataDir(dataDir))
}
