...
function createModelClient(input: { baseUrl: string; apiKey: string; model: string }): ModelClient {
  if (isAnthropicCompatibleBaseUrl(input.baseUrl)) {
    return new AnthropicCompatModelClient(input)
  }
  return new DeepseekCompatModelClient(input)
}
...
function isAnthropicCompatibleBaseUrl(baseUrl: string): boolean {
  try {
    ...
    return host === 'api.kimi.com' && url.pathname.startsWith('/coding')
  } catch {
