...
import {
  checkDataComplianceEnvironment,
  createDataComplianceTask,
  deleteDataComplianceTask,
  downloadDataComplianceTaskFile,
  getDataComplianceInputFile,
  getDataComplianceTask,
  getDataComplianceTaskProgress,
  listDataComplianceTasks
 ...
 * - `POST /v1/knowledge/sync`, search, agent sources, diagnostics (auth)
...
export function buildRouter(runtime: ServerRuntime): Router {
  ...
  router.add('POST', '/v1/knowledge/sync', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return syncKnowledge(runtime.knowledgeStore, request)
  })
  router.add('GET', '/v1/knowledge/search', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return searchKnowledge(runtime.knowledgeStore, request)
  })
  router.add('GET', '/v1/knowledge/agent-sources', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
  ...
  router.add('GET', '/v1/knowledge/tree', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return knowledgeTree(runtime.knowledgeStore, request)
  })
  router.add('POST', '/v1/knowledge/folder', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return knowledgeCreateFolder(runtime.knowledgeStore, request)
  })
  router.add('POST', '/v1/knowledge/file', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return knowledgeWriteFile(runtime.knowledgeStore, request)
  })
  ...
  router.add('GET', '/v1/knowledge/file/absolute-path', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return knowledgeAbsolutePath(runtime.knowledgeStore, request)
  })
  ...
  router.add('DELETE', '/v1/knowledge/file', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return knowledgeDelete(runtime.knowledgeStore, request)
  })
  ...
  router.add('GET', '/data-compliance/environment', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return checkDataComplianceEnvironment(runtime.dataComplianceTaskService)
  })
  router.add('POST', '/data-compliance/tasks', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return createDataComplianceTask(runtime.dataComplianceTaskService, request)
  })
  router.add('GET', '/data-compliance/tasks', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return listDataComplianceTasks(runtime.dataComplianceTaskService)
  })
  router.add('GET', '/data-compliance/tasks/:id', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return getDataComplianceTask(runtime.dataComplianceTaskService, ctx.params.id)
  })
  router.add('DELETE', '/data-compliance/tasks/:id', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return deleteDataComplianceTask(runtime.dataComplianceTaskService, ctx.params.id)
  })
  router.add('GET', '/data-compliance/tasks/:id/progress', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return getDataComplianceTaskProgress(runtime.dataComplianceTaskService, ctx.params.id)
  })
  router.add('GET', '/data-compliance/tasks/:id/files/:fileKey', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return downloadDataComplianceTaskFile(
      runtime.dataComplianceTaskService,
      ctx.params.id,
      ctx.params.fileKey
    )
  })
  router.add('GET', '/data-compliance/tasks/:id/input', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    if (!runtime.dataComplianceTaskService) {
      return ERRORS.unavailable('data compliance service is not available')
    }
    return getDataComplianceInputFile(runtime.dataComplianceTaskService, ctx.params.id)
  })
