...
export const GuiPlanContextSchema = z.object({
  ...
  relativePath: z
    ...
    .refine(isGuiPlanRelativePath, {
      message: 'relativePath must be a direct Markdown file under .legalworksdd/plan'
    }),
...
export const StartTurnRequest = z.object({
   ...
   * mode Legalwork advertises `create_plan` for the whole conversation.
   */
  mode: TurnModeSchema.optional(),
  attachments: z
    .array(
  ...
  attachmentIds: z.array(z.string().min(1)).default([]),
  /**
   * Optional GUI plan context. When set, Legalwork advertises the
