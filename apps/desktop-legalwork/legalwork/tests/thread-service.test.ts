...
import { DEFAULT_LEGALWORK_MODEL } from '../src/config/legalwork-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'legalwork-todos-'))
    try {
      const relativePath = '.legalworksdd/plan/demo.md'
...
describe('ThreadService.resumeSession', () => {
  it('uses the Legalwork default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_LEGALWORK_MODEL)
  })
})
