...
describe('Legalwork agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'legalwork-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitLegalworkCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitLegalworkCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitLegalworkCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from legalwork exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through legalwork exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from legalwork exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'legalwork.config.json')
