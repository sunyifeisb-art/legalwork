...
describe('Legalwork built-in tools', () => {
  let workspace: string
  let host: LocalToolHost

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'legalwork-tools-'))
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello legalwork' },
    ...
    expect(editedDisk).toContain('hello legalwork')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello legalwork')
