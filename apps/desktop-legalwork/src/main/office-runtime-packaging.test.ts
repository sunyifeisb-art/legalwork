...
describe('bundled Office runtime packaging contract', () => {
  ...
  it('rejects a Windows Office runtime that embeds the separate data-compliance environment', () => {
    const root = tempRoot()
    const context = {
    ...
    expect(() => afterPack._internals.validateBundledOfficeRuntime(context)).toThrow(/must not embed/)
  })

  it('accepts the separately prepared 32-bit Windows Office runtime', () => {
    const root = tempRoot()
    const context = {
      appOutDir: join(root, 'win-ia32'),
      electronPlatformName: 'win32',
      arch: 'ia32',
      packager: { appInfo: { productFilename: 'legalwork' }, projectDir: join(root, 'project') }
    }
