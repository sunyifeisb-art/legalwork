export const GUI_PLAN_RELATIVE_DIR = '.legalworksdd/plan'
...
/**
 * Stable name of the native Legalwork plan tool. Kept distinct from the
 * historical `gui_plan_create` MCP bridge so the renderer and Legalwork
 ...
 * plan/refine turn so Legalwork can scope tool availability to the
 * active plan context.
 */
 ...
 * the public surface the model sees; validation is enforced by Legalwork
...
/**
 * Build the deterministic plan id used by both renderer and Legalwork.
...
export function validateCreatePlanToolInput(input: Partial<CreatePlanToolInput>): string[] {
  ...
  if (input.plan_relative_path != null) {
    ...
    if (!path) {
      ...
      issues.push('plan_relative_path must be a direct Markdown file under .legalworksdd/plan')
    }
  }
 ...
 * plan path checks. Used by Legalwork to verify the active workspace
