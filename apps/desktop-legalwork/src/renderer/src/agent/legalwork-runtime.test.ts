...
describe('LegalworkRuntimeProvider', () => {
  it('reports the legalwork id and Legalwork display name', () => {
    const provider = new LegalworkRuntimeProvider()
    expect(provider.id).toBe('legalwork')
    expect(provider.displayName).toBe('Legalwork')
  })
  ...
  it('posts Legalwork turn requests and returns the deterministic user item id', async () => {
  ...
  it('posts attachment ids with Legalwork turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts explicit reasoning effort with Legalwork turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts GUI plan context with Legalwork plan turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
    ...
    await provider.sendUserMessage('thr_1', 'refine the plan', {
      ...
      guiPlan: {
        ...
        planId: '/workspace/legalwork:.legalworksdd/plan/auth.md',
  ...
  it('loads runtime diagnostics and uploads image attachments through Legalwork endpoints', async () => {
  ...
  it('resumes a session through the Legalwork HTTP runtime', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 201,
    ...
    expect(runtimeRequest).toHaveBeenCalledWith(
      ...
      JSON.stringify({
        workspace: '/tmp/workspace',
        model: defaultLegalworkRuntimeSettings().model,
        mode: 'plan'
      })
    )
  })
  ...
  it('auto-approves approval requests when policy is auto', async () => {
    ...
    const autoSettings: AppSettingsV1 = {
      ...settings(),
      agents: { legalwork: { ...defaultLegalworkRuntimeSettings(), approvalPolicy: 'auto' } }
    }
    installDsGui({
  ...
  it('renders approval cards for suggest and untrusted policies', async () => {
    for (const policy of ['suggest', 'untrusted'] as const) {
      ...
      const policySettings: AppSettingsV1 = {
        ...settings(),
        agents: { legalwork: { ...defaultLegalworkRuntimeSettings(), approvalPolicy: policy } }
      }
      installDsGui({
...
describe('registry', () => {
  it('returns a cached provider for the legalwork id', () => {
