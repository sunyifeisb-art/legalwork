...
### v0.3.30-rch.1（2026-09-01）

同步上游 0.3.30/0.3.31 修复，保留 RCH 的完整离线依赖和 Windows 数据合规环境。

- Windows 运行时与数据合规安装流程进一步修复，新增 Python 安装源和 Office 运行时修复。
- Filesystem MCP 改为本地离线启动，降低网络波动导致的连接超时。
- 加固 Agent、知识库、文书 Skill、插件市场及跨平台运行时稳定性。
- RCH 安装包统一命名为 `LegalWork-0.3.30-RCH-win-x64.exe`。
...
本次 RCH 版本同步上游 LegalWork 0.3.28 及后续修复：

- 支持 DeepSeek V4-Flash-Vision-Exp 视觉模型，图片可直接传给模型分析，不再依赖 OCR 文本回退。
- 模型选择器完整显示长模型名，推理强度标签不再与模型名重叠。
- 新对话默认推理强度调整为“高”。
- 网络检索失败时自动降级，不再阻塞整轮任务；模型可继续使用附件和本地知识完成回答。
- 保留 RCH 的离线依赖、Windows 数据合规环境修复和独立更新通道。

安装包和 Release 资产统一使用 `RCH` 后缀，例如 `LegalWork-0.3.28-RCH-win-x64.exe`。
