...
function defaultModelProviderSettings() {
  ...
  return {
    ...
    providers: BUILTIN_MODEL_PROVIDER_PRESETS.map(
      (preset) => preset.id === DEFAULT_MODEL_PROVIDER_ID ? defaultProvider : defaultModelProviderProfile("", preset.baseUrl, preset.id, preset.name, preset.models)
    )
  };
}
function normalizeModelProviderSettings(input) {
  ...
  for (const preset of BUILTIN_MODEL_PROVIDER_PRESETS) {
    providersById.set(
      preset.id,
      preset.id === DEFAULT_MODEL_PROVIDER_ID ? defaultProvider : defaultModelProviderProfile("", preset.baseUrl, preset.id, preset.name, preset.models)
    );
  }
...
function defaultModelProviderProfile(apiKey, baseUrl, id = DEFAULT_MODEL_PROVIDER_ID, name = getBuiltinModelProviderPreset(DEFAULT_MODEL_PROVIDER_ID)?.name ?? DEFAULT_MODEL_PROVIDER_NAME, models = getBuiltinModelProviderPreset(DEFAULT_MODEL_PROVIDER_ID)?.models ?? DEFAULT_COMPOSER_MODEL_IDS.filter((modelId) => modelId !== "auto")) {
  return {
    id,
    name,
    apiKey: apiKey.trim(),
...
const McpServerConfig = zod.z.object({
  ...
  if (server2.trustScope === "workspace" && server2.trustedWorkspaceRoots.length === 0) {
    ctx.addIssue({
      code: "custom",
...
const AttachmentsCapabilityConfig = CapabilityToggleConfig.extend({
  ...
  allowedMimeTypes: zod.z.array(zod.z.string().min(1)).default(["image/png", "image/jpeg", "image/webp"]),
    ...
    try {
      ...
      if (!pythonCmd) {
        sendProgress({
          step: "error",
          percent: 0,
          message: process.platform === "win32" ? "未找到 Python 3，自动安装失败。请手动安装 Python 3 并确保其在 PATH 中。" : "未找到 Python 3，请先安装 Python 3 并确保其在 PATH 中。"
        });
        return false;
      }
...
async function loadGuiUpdaterModule() {
  if (!guiUpdaterModulePromise) {
    guiUpdaterModulePromise = Promise.resolve().then(() => require("./chunks/gui-updater-9f7dA_4M.cjs")).then((module2) => {
