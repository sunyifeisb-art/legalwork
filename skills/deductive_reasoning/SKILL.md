...
description: |
  ...
  注意：本技能输出的是逻辑分析格式（大前提-小前提-结论），主要用于内部推理校验；起草上诉状、代理词、辩护词等诉讼文书时，不应机械套用此格式，而应转化为自然、有说服力的法律语言。
---
...
**重要提示：** 本技能输出的是逻辑分析格式（大前提-小前提-结论），主要用于内部推理校验；起草上诉状、代理词、辩护词等诉讼文书时，不应机械套用此格式，而应转化为自然、有说服力的法律语言。诉讼文书应围绕争议焦点，先阐明主张，再结合事实、证据与法律依据展开论证。
...
### 第三步：输出 
每次调用本技能，必须严格按照以下格式输出两部分内容（本输出为逻辑分析草稿，不应直接作为上诉状、代理词等诉讼文书的正文）：
#### 1. 可解释的推理链条
使用清晰的 P-F-C 结构图或层级列表，展示结论是如何通过中间结论、法律小前提、自然小前提一步步回溯、链接到相应大前提的。明确标出每一步所使用的中项。

> **注意：** 若本技能的结果用于起草诉讼文书，应将 P-F-C 结构转化为自然语言论证：先提出主张，再陈述关键事实与证据，最后援引法律规范得出结论。不得在文书中逐条标注“大前提”“小前提”“结论”。
