...
import { createLegalworkServeRuntime } from '../server/runtime-factory.js'
...
export const LEGALWORK_CLI_USAGE = `legalwork <command> [options]
...
Common options:
  ...
  --data-dir <path>          Root directory for Legalwork data
...
export type LegalworkCliCommand = 'serve' | 'run' | 'chat' | 'exec' | 'help'

export function splitLegalworkCliCommand(argv: readonly string[]): {
  command: LegalworkCliCommand
  args: string[]
  error?: string
} {
  const first = argv[0]
...
export async function runAgentCommand(
  command: Exclude<LegalworkCliCommand, 'serve' | 'help'>,
...
async function runOneShot(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  if (!parsed.ok) return writeParseError(parsed, io, 'legalwork run')
  const prompt = stringFlag(argv, ['prompt', 'p']) ?? positionals(argv).join(' ').trim()
  if (!prompt) {
    io.stderr.write('legalwork run: missing prompt\n')
  ...
  try {
    ...
    io.stderr.write(`legalwork run: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'legalwork run')
  }
}

async function runChat(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'legalwork chat')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
    const thread = await runtime.threadService.create({
    ...
    io.stderr.write(`legalwork chat: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'legalwork chat')
  }
}
...
async function runExec(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'legalwork exec')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
  } catch (error) {
    io.stderr.write(`legalwork exec: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  }
  ...
  try {
    ...
    if (!toolName) {
      io.stderr.write('legalwork exec: missing tool name (use --list-tools to inspect tools)\n')
      return ServeExitCode.usage
    }
    const argsText = stringFlag(argv, ['args']) ?? '{}'
    const args = parseJsonObject(argsText)
    if (!args.ok) {
      io.stderr.write(`legalwork exec: ${args.message}\n`)
    ...
    await shutdownRuntime(runtime, io, 'legalwork exec')
  }
}
...
function parseSharedOptions(argv: readonly string[], io: CliIo): SharedOptionsResult {
  ...
  return {
    ...
    workspace: stringFlag(argv, ['workspace']) ?? io.env?.LEGALWORK_WORKSPACE ?? io.cwd?.() ?? process.cwd(),
    json: hasFlag(argv, 'json')
  }
}

function createRuntime(options: ServeOptions, io: CliIo): Promise<ServerRuntime> {
  return io.createRuntime ? io.createRuntime(options) : createLegalworkServeRuntime(options)
}
