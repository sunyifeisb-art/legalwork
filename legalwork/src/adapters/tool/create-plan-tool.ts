...
const TOOL_DESCRIPTION = [
  'Create or replace a legalwork-owned implementation plan.',
  ...
  '.legalworksdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_id: {
      type: 'string',
      description: 'Optional reserved plan id; when supplied, must match the legalwork plan context.'
    },
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .legalworksdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.legalworksdd/plan/foo.md`)
 ...
 *    `.legalworksdd/plan/<feature>.md` under the active workspace.
...
export async function executeCreatePlanTool(
  ...
  if (!isPlanToolContextActive(context)) {
    return {
      output: { error: 'create_plan requires Plan mode or an active legalwork plan context' },
      isError: true
    }
  }
  ...
  const output: CreatePlanToolOutput = {
    summary: `${resolved.operation === 'refine' ? 'Refined' : 'Created'} legalwork plan at ${resolved.relativePath}.`,
...
function resolveReservedTarget(
  ...
  if (!contextPlan) {
    return { error: 'create_plan requires an active legalwork plan context' }
  }
  if (input.operation !== contextPlan.operation) {
    return { error: 'operation does not match the active legalwork plan operation' }
  }
  if (!guiPlanWorkspaceMatches(context.workspace, contextPlan.workspaceRoot)) {
    return { error: 'tool workspace does not match the active legalwork plan workspace' }
  }
  const relativePath = toRelativePath(contextPlan.relativePath)
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .legalworksdd/plan' }
  }
  ...
  if (input.plan_relative_path && toRelativePath(input.plan_relative_path) !== contextPlan.relativePath) {
    return { error: 'plan_relative_path does not match the reserved legalwork plan path' }
  }
  if (input.plan_id && input.plan_id !== contextPlan.planId) {
    return { error: 'plan_id does not match the reserved legalwork plan id' }
  }
 ...
 * allocates a fresh, non-colliding `.legalworksdd/plan/<feature>.md`.
 */
