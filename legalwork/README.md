...
Legalwork is the local HTTP/SSE agent runtime for DeepSeek-GUI. It exposes a
TypeScript-typed agent loop with a stable, GUI-friendly contract:

- `legalwork serve` starts a local HTTP server with `/v1/*` routes.
...
Legalwork is a deeper local runtime rather than a thin model UI: one
...
Legalwork's core goal is to improve the ROI of every token. Tokens should be
...
Run from the `legalwork/` directory.
...
`legalwork serve` accepts the following flags:

| Flag | Description | Default |
| --- | --- | --- |
| `--config` | JSON config file. If omitted, Legalwork reads `{--data-dir}/config.json` when present | optional |
...
```bash
legalwork serve \
  --config ~/.legalwork/legalwork/config.json \
  --host 127.0.0.1 \
  --port 8899 \
  --data-dir ~/.legalwork/legalwork \
  --runtime-token dev-token \
  --api-key "$DEEPSEEK_API_KEY" \
  --model deepseek-v4-pro
```

Legalwork can also run as a standalone agent without the GUI:

```bash
legalwork run --data-dir ~/.legalwork/legalwork --workspace "$PWD" "summarize this repo"
legalwork chat --data-dir ~/.legalwork/legalwork --workspace "$PWD"
legalwork exec --data-dir ~/.legalwork/legalwork --workspace "$PWD" --list-tools
legalwork exec --data-dir ~/.legalwork/legalwork --workspace "$PWD" read --args '{"path":"README.md"}'
```

- `legalwork run` creates a thread, runs one turn, streams assistant text, and exits.
- `legalwork chat` starts a line-oriented REPL. Use `/exit`, `/quit`, or an empty line to stop.
- `legalwork exec --list-tools` prints the effective dynamic tool registry for the chosen config/workspace.
- `legalwork exec <tool> --args <json>` invokes one tool directly. Use `--json` on `run` or `exec` for machine-readable output.

## Environment variables

The runtime reads these from `process.env` when not set via CLI flags.

- `LEGALWORK_CONFIG` – explicit JSON config file
- `LEGALWORK_HOST` – bind host (overrides `--host` if set)
- `LEGALWORK_PORT` – bind port (overrides `--port` if set)
- `LEGALWORK_DATA_DIR` – root data directory (overrides `--data-dir` if set)
- `LEGALWORK_RUNTIME_TOKEN` – bearer token (overrides `--runtime-token` if set)
- `LEGALWORK_BASE_URL` – model API base URL (overrides `--base-url` if set)
- `DEEPSEEK_BASE_URL` – fallback model API base URL
- `LEGALWORK_MODEL` – default model id (overrides `--model` if set)
...
Legalwork supports a JSON config file so runtime behavior can be managed
...
Use `--config <path>` or `LEGALWORK_CONFIG=<path>` for an explicit file. If
no explicit config is provided and `--data-dir` / `LEGALWORK_DATA_DIR` is set,
Legalwork also reads `{data-dir}/config.json` when it exists. In the GUI's
default setup this is:

```text
~/.legalwork/legalwork/config.json
```
...
Legalwork defaults to hybrid session storage: `threads/{threadId}/messages.jsonl`
...
See `../docs/LEGALWORK_CONFIG.md` for the detailed file layout and examples.
...
```
{--data-dir}/
  config.json      # Optional Legalwork runtime config
...
3. Restart Legalwork or refresh diagnostics.
...
- Memory is not injected: enable `capabilities.memory`, confirm
  ...
  `lastInjectedIds`.
- `legalwork run`, `legalwork chat`, or `legalwork exec` cannot authenticate or load
  config: pass the same `--config`, `--data-dir`, `--api-key`,
  `--base-url`, and `--runtime-token` values used by `legalwork serve`.
  `legalwork exec --list-tools --json` is the quickest way to verify the
...
starts Legalwork through `legalwork-process.ts` and routes all
...
CodeWhale provider because Legalwork speaks the same HTTP/SSE
contract. Settings live under `agents.legalwork` in
...
`agents.legalwork`; the saved settings file no longer keeps live
